...
static bool js_cocos2dx_extension_EventAssetsManagerEx_getDownloadedFiles(JSContext *cx, uint32_t argc, jsval *vp)
{
...
bool js_cocos2dx_extension_Manifest_constructor(JSContext *cx, uint32_t argc, jsval *vp)
{
	...
	do {
		if (argc == 2) {
			...
			ok &= jsval_to_std_string(cx, args.get(0), &arg0);
			ok &= jsval_to_std_string(cx, args.get(1), &arg1);
			JSB_PRECONDITION2(ok, cx, false, "js_cocos2dx_extension_Manifest_constructor : Error processing arguments");
			...
			// link the native object with the javascript object
			JS::RootedObject jsobj(cx, jsb_ref_create_jsobject(cx, cobj, typeClass, "cocos2d::extension::Manifest"));
			args.rval().set(OBJECT_TO_JSVAL(jsobj));
			if (JS_HasProperty(cx, jsobj, "_ctor", &ok) && ok)
				ScriptingCore::getInstance()->executeFunctionWithOwner(OBJECT_TO_JSVAL(jsobj), "_ctor", args);
			return true;
...
static bool js_cocos2dx_extension_AssetsManagerEx_loadLocalManifest(JSContext *cx, uint32_t argc, jsval *vp)
{
...
void js_register_cocos2dx_extension_AssetsManagerEx(JSContext *cx, JS::HandleObject global) {
    ...
    static JSFunctionSpec funcs[] = {
		...
		JS_FN("loadLocalManifest", js_cocos2dx_extension_AssetsManagerEx_loadLocalManifest, 0, JSPROP_PERMANENT | JSPROP_ENUMERATE),
        JS_FS_END
    };

    static JSFunctionSpec st_funcs[] = {
        JS_FN("create", js_cocos2dx_extension_AssetsManagerEx_create, 0, JSPROP_PERMANENT | JSPROP_ENUMERATE),
        JS_FS_END
    };
