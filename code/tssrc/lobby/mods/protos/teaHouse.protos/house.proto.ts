namespace tea {
    ...
    export interface proto_housememberonline_res extends IBASE_MESSAGE{
     ...
     export interface HouseMixFloorInfo extends IBASE_MESSAGE {   //混排信息返回
        is_mix: boolean,                          //是否混排
        mix_active: boolean,                      //
        table_num: number,                        //初始桌子数       
        fids: Array<number>,                      //混的楼层
        house_table_join_type: number,                         //混排模式
        ai_check: boolean,
        ai_total_score_limit: number              //数值设定
        ai_super: boolean,                        //超级防作弊           
        empty_table_back: boolean                  //空桌位置
        empty_table_max: number                    //最大的空桌数
        table_sort_type: number                     //自动加桌模式下  排序方式；  
        new_table_sort_type:number                  //自动加桌模式新的排序方式
        create_table_type:number                    //开桌方式
    }
