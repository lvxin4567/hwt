namespace tea {
    ...
    export class tea_FcmBatchPanelMgr {
        static __INS__: tea_FcmBatchPanelMgr = null;
        static getInstance(_zOrder: number = 0) {
            if (tea_FcmBatchPanelMgr.__INS__ == null) {
                tea_FcmBatchPanelMgr.__INS__ = new tea_FcmBatchPanelMgr();
                tea_FcmBatchPanelMgr.__INS__.init();
                tea_FcmBatchPanelMgr.__INS__._zOrder = _zOrder;
            }
            return tea_FcmBatchPanelMgr.__INS__;
        }
        __selfPanel: FcmBatchPanel = null;
        _zOrder: number = 0;
        init() {
            let self = this;
            this.__selfPanel = null;

            kaayou.getController('tea').on('ui::FcmBatchPanel::Show', function (e: kaayou.Event) {
                self.getPanel(true).Show(e.data);
            }, this, 10);
            kaayou.getController('tea').on('ui::FcmMember::ShowData', function (e: kaayou.Event) {
                self.getPanel(false) && self.getPanel(false).onUpdateMember(e.data);
            }, this, 10);
        ...
        getPanel(create: boolean = false) {
            if (create && this.__selfPanel == null) {
                this.__selfPanel = new FcmBatchPanel();
    ...
    export class FcmBatchPanel extends kaayou.ModelLayer {
        _partnerId: number = 0;
        ...
        initUI() {
            let self = this;
            this.initWithccs(tea.res.FcmBatch_json);
        ...
        doGetMemList(clear: boolean = true, sorttype: number = 0) {
            let self = this;
            self.sortType = sorttype;
            let search = this.ebLeftSearch.getString().length < 1 ? "" : this.ebLeftSearch.getString();
            kaayou.emit("tea", 'mod::TeaHouse::GetFcmMember', { hid:tea.mod.__teaHouseInfo.hid });
        }
        ...
        onUpdateMember(data: { list, update: boolean}) {
            let self = this;
            if (!!data.list) {
