namespace tea {
    ...
    export class tea_matchRewardteamConfigDialogMgr{
        static  __INS__: tea_matchRewardteamConfigDialogMgr = null;
        static getInstance(_zOrder: number = 0) {
            if (tea_matchRewardteamConfigDialogMgr.__INS__ == null) {
                tea_matchRewardteamConfigDialogMgr.__INS__ = new tea_matchRewardteamConfigDialogMgr();
                tea_matchRewardteamConfigDialogMgr.__INS__.init();
                tea_matchRewardteamConfigDialogMgr.__INS__._zOrder = _zOrder
            }
            return tea_matchRewardteamConfigDialogMgr.__INS__;
        }
        _zOrder: number = 0
        __selfDialog:MatchRewardteamConfigDialog = null

        init() {
            let self = this;
            this.__selfDialog = null;

            kaayou.getController('tea').on('ui::matchRewardteamConfigDialog::Show', function (e: kaayou.Event) {
                self.getPanel(true).Show();
            }, this, 10);

            kaayou.getController('tea').on('ui::matchRewardteamConfigDialog::Hide', function (e: kaayou.Event) {
        ...
        getPanel(create: boolean = false) {
            if (create && this.__selfDialog == null) {
                this.__selfDialog = new MatchRewardteamConfigDialog();
    ...
    class MatchRewardteamConfigDialog extends kaayou.Layer{
        ...
        private listArray:Array<MatchRewardteamConfigDialogItem> = null;
        ...
        initUI(){
            ...
            this.closeNode.on(kaayou.TouchEvent.TouchEnd , ()=>{
                kaayou.emit("tea" , "ui::matchRewardteamConfigDialog::Hide")
            },this)
            ...
            this.title.setString("战队赛奖励设置")

        }

        async pullConfigList(){
            let hid = tea.mod.__teaHouseInfo.hid;
            let data =  await kaayou.sendMessage("lobby","houseprizeinfo", {hid,type:2},"ws::Msg::houseprizeinfo");
        ...
        private async submitConfigList(){
            ...
            let data = await kaayou.sendMessage("lobby","houseprizeset", {hid,value,type:2},"ws::Msg::houseprizeset");
        ...
        initList(){
            ...
            for(let i =0; i < 20 ;i++){
                let node = new MatchRewardteamConfigDialogItem(this.item)
                node.setIndex(i);
                node.setRewardInfo();
                node.setInfo({num:-1});
